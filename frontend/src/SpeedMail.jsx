...
import { Mail, Copy, RefreshCw, Trash2, Clock, Check, Inbox, ArrowLeft, Zap } from 'lucide-react';
  ...
  const generateEmail = async () => {
    ...
    try {
      if (DEMO_MODE) {
        ...
        const demoEmail = `${randomStr}@speedmail.temp`;
      ...
      if (DEMO_MODE) {
        if (inbox.length === 0) {
          const demoEmails = [
            {
              id: '1',
              from: 'welcome@speedmail.com',
              subject: 'Welcome to SpeedMail!',
              body: 'SpeedMail - Ultra-fast temporary email delivery!',
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                <div className="bg-gradient-to-br from-orange-600 to-amber-600 p-1.5 sm:p-2 rounded-lg sm:rounded-xl shadow-sm">
                ...
                <div>
                  <h1 className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
                    SpeedMail<sup className="text-xs sm:text-sm ml-0.5 text-orange-600">GB</sup>
                  </h1>
                  <p className="text-xs sm:text-sm text-gray-600">Ultra-Fast Delivery</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200/60 p-4 sm:p-8 mb-4 sm:mb-6">
          <div className="space-y-3 sm:space-y-5">
            <div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2.5 bg-gray-50/50 border border-gray-300 p-2 sm:p-2.5 rounded-xl focus-within:border-orange-600 focus-within:ring-2 sm:focus-within:ring-4 focus-within:ring-orange-100 transition-all">
                <div className="flex items-center space-x-2 flex-1 min-w-0">
                  <Zap className="w-4 h-4 text-orange-400 flex-shrink-0" />
                  <input type="text" value={currentEmail} readOnly className="flex-1 bg-transparent text-sm sm:text-base font-mono font-semibold outline-none text-gray-900 selection:bg-orange-100 min-w-0" />
                </div>
                <button onClick={copyToClipboard} className="flex-shrink-0 px-3 py-2 sm:py-1.5 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 text-white rounded-lg transition-all text-sm font-medium flex items-center justify-center space-x-2">
                  {copied ? <><Check className="w-4 h-4" /><span>Copied</span></> : <><Copy className="w-4 h-4" /><span>Copy</span></>}
                </button>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
              <div className="bg-gradient-to-br from-orange-50 to-amber-100/50 border border-orange-200/60 p-3 sm:p-4 rounded-lg sm:rounded-xl">
                <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 border border-amber-200/60 p-3 sm:p-4 rounded-lg sm:rounded-xl">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  <div className="p-1.5 sm:p-2 bg-white rounded-lg shadow-sm">
                    <Inbox className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" />
                  </div>
                  <div>
                    <p className="text-xs text-amber-600/80 font-medium">Emails received</p>
                    <p className="text-lg sm:text-xl font-semibold text-amber-900">{inbox.length}</p>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-1 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200/60 overflow-hidden">
            <div className="border-b border-gray-200/60 px-3 sm:px-5 py-3 sm:py-4 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="px-1.5 sm:px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-medium rounded-full">{inbox.length}</span>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-3">
                <div className="flex items-center space-x-1 px-1.5 sm:px-2.5 py-1 bg-orange-50 border border-orange-200 rounded-md sm:rounded-lg">
                  <span className="text-xs font-semibold text-orange-700">{countdown}s</span>
                </div>
            ...
            <div className="divide-y divide-gray-100 max-h-[400px] sm:max-h-[500px] overflow-y-auto">
              {inbox.length === 0 ? (
                <div className="p-8 sm:p-12 text-center">
                  <div className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 bg-orange-100 rounded-xl mb-3">
                ...
                inbox.map((email) => (
                  <div key={email.id} onClick={() => markAsRead(email)} className={`p-3 sm:p-4 cursor-pointer hover:bg-gray-50 transition-all border-l-2 ${selectedEmail?.id === email.id ? 'bg-orange-50/50 border-orange-600' : 'border-transparent'}`}>
                    ...
                    {!email.read && (
                      <span className="inline-flex items-center text-xs bg-orange-600 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full font-medium">
            ...
            {selectedEmail ? (
              ...
              <div className="h-64 sm:h-96 flex items-center justify-center p-4">
                <div className="text-center">
                  <div className="inline-block p-4 sm:p-6 bg-orange-100 rounded-full mb-3">
                    <Zap className="w-12 h-12 sm:w-16 sm:h-16 text-orange-400" />
                  </div>
      ...
      <footer className="mt-12 pb-8 border-t border-gray-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 pt-6 text-center">
          <div className="flex items-center justify-center space-x-2 mb-2">
            <div className="bg-gradient-to-br from-orange-600 to-amber-600 p-1.5 sm:p-2 rounded-lg">
              <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
            </div>
            <h3 className="text-lg sm:text-xl font-bold text-gray-900">SpeedMail<sup className="text-xs ml-0.5 text-orange-600">GB</sup></h3>
          </div>
          <p className="text-gray-600 text-xs sm:text-sm">© 2024 SpeedMail<sup className="text-[10px] text-orange-600">GB</sup> - Ultra-Fast Delivery</p>
        </div>
      </footer>
    </div>
  );
